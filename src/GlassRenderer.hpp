...
extern const CBox* g_pActiveScissorClip;
...
void applyGlassEffect(SP<Render::IFramebuffer> sampleFramebuffer, SP<Render::IFramebuffer> targetFramebuffer,
                       ...
                       const GlassShadowPayload* shadow = nullptr,
                       // Maps the true (unpadded) region's bounds as a UV sub-rect within
                       // rawBox/transformedBox, when those have been inflated for outer
                       // shadow. Defaults are a no-op remap (whole box is the true box).
                       const Vector2D& trueBoxUVOffset = Vector2D{0.0, 0.0},
                       const Vector2D& trueBoxUVScale = Vector2D{1.0, 1.0});
