...
const CBox* g_pActiveScissorClip = nullptr;
...
void applyGlassEffect(SP<Render::IFramebuffer> sampleFramebuffer, SP<Render::IFramebuffer> targetFramebuffer,
                       ...
                       const SMaskInfo* mask, const GlassShadowPayload* shadow,
                       const Vector2D& trueBoxUVOffset, const Vector2D& trueBoxUVScale) {
    ...
    static const auto glassStartTime = std::chrono::steady_clock::now();
    const float glassElapsedTime = std::chrono::duration<float>(std::chrono::steady_clock::now() - glassStartTime).count();
    glUniform1f(uniforms.u_time, glassElapsedTime);
    ...
    glUniform1f(uniforms.reflectionStrength, resolvePresetFloat(resolveContext, &SPresetValues::reflectionStrength, &SOverridableConfig::reflectionStrength));
    glUniform1f(uniforms.refractThickness, resolvePresetFloat(resolveContext, &SPresetValues::refractThickness, &SOverridableConfig::refractThickness));
    glUniform1f(uniforms.refractIOR, resolvePresetFloat(resolveContext, &SPresetValues::refractIOR, &SOverridableConfig::refractIOR));
    glUniform1f(uniforms.refractScale, resolvePresetFloat(resolveContext, &SPresetValues::refractScale, &SOverridableConfig::refractScale));
    ...
    glUniform1f(uniforms.shadowBlurStrength,  resolvePresetFloat(resolveContext, &SPresetValues::blurStrength, &SOverridableConfig::blurStrength));
    ...
    // Separate from and additive to the hardcoded bottom-rim shadow.
    if (shadow && shadow->hasInner) {
        glUniform1f(uniforms.shadowHasInner, 1.0f);
        glUniform1f(uniforms.shadowInnerBlur, shadow->innerBlur);
        glUniform1f(uniforms.shadowInnerSpread, shadow->innerSpread);
        glUniform4f(uniforms.shadowInnerColor, shadow->innerR, shadow->innerG, shadow->innerB, shadow->innerA);
        glUniform2f(uniforms.shadowInnerOffset, shadow->innerOffsetX, shadow->innerOffsetY);
    } else {
        glUniform1f(uniforms.shadowHasInner, 0.0f);
    }

    // User-configurable per-GlassItem outer shadow. Paints outside the true
    // region edge, into the padded draw quad GlassLayerSurface inflated for
    // this region when shadow->hasOuter is set.
    if (shadow && shadow->hasOuter) {
        glUniform1f(uniforms.shadowHasOuter, 1.0f);
        glUniform1f(uniforms.shadowOuterBlur, shadow->outerBlur);
        glUniform1f(uniforms.shadowOuterSpread, shadow->outerSpread);
        glUniform4f(uniforms.shadowOuterColor, shadow->outerR, shadow->outerG, shadow->outerB, shadow->outerA);
        glUniform2f(uniforms.shadowOuterOffset, shadow->outerOffsetX, shadow->outerOffsetY);
        glUniform1f(uniforms.shadowOuterIsTrue, shadow->outerIsTrue ? 1.0f : 0.0f);
    } else {
        glUniform1f(uniforms.shadowHasOuter, 0.0f);
        glUniform1f(uniforms.shadowOuterIsTrue, 0.0f);
    }

    glUniform2f(uniforms.trueBoxUVOffset,
        static_cast<float>(trueBoxUVOffset.x),
        static_cast<float>(trueBoxUVOffset.y));
    glUniform2f(uniforms.trueBoxUVScale,
        static_cast<float>(trueBoxUVScale.x),
        static_cast<float>(trueBoxUVScale.y));

    shader->setUniformFloat(SHADER_RADIUS, cornerRadius);
    shader->setUniformFloat(SHADER_ROUNDING_POWER, roundingPower);

    glBindVertexArray(shader->getUniformLocation(SHADER_SHADER_VAO));
    if (g_pActiveScissorClip) {
        CBox scissorBox = rawBox.intersection(*g_pActiveScissorClip);
